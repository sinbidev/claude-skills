...
3. Run the following command directly (no interactive input needed):

```
openssl req -new -x509 -newkey rsa:4096 -keyout sdf-credentials/private.pem -sigopt rsa_padding_mode:pss -sha256 -sigopt rsa_pss_saltlen:64 -out sdf-credentials/public.pem -nodes -days 730 -subj "/C=US"
```

4. After the command completes, verify that `private.pem` and `public.pem` exist inside `sdf-credentials/`. Remind the user that this folder is git-ignored and should never be committed.
